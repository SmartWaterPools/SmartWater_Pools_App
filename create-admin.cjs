...
async function createSystemAdmin() {
  try {
    ...
    if (existingUser.rows.length > 0) {
      ...
      await pool.query(
        'UPDATE users SET password = $1, role = $2 WHERE username = $3 OR email = $4',
        [hashedPassword, 'system_admin', 'Travis@smartwaterpools.com', 'Travis@smartwaterpools.com']
      );
      
      console.log('User updated successfully to system_admin with the new password');
      return;
    }
    
    // Create a new organization if needed (assuming first org is the default)
    ...
    if (orgResult.rows.length > 0) {
      ...
      const newOrg = await pool.query(
        'INSERT INTO organizations (name, slug, plan_type, is_active) VALUES ($1, $2, $3, $4) RETURNING id',
        ['SmartWater Pools', 'smartwater-pools', 'enterprise', true]
      );
    ...
    const result = await pool.query(
      `INSERT INTO users (
        ...
        created_at,
        updated_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW()) RETURNING id, username`,
      [
